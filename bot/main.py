...
def getrepllogsCommand(update: Update, context):
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    client.connect(hostname=db_host, username="postgres", password="Qq12345", port=22)
    stdin, stdout, stderr = client.exec_command('tail -n 20 grep "replication" /var/log/postgresql/postgresql.log')
    data = stdout.read() + stderr.read()
    client.close()
    data = str(data).replace('\\n', '\n').replace('\\t', '\t')[2:-1]
    update.message.reply_text(data)
